...
// init menginisialisasi validator saat package di-load
func init() {
	InitValidator()
}
...
// ValidateRegisterInput memvalidasi dan membersihkan RegisterRequest
func ValidateRegisterInput(req *domain.RegisterRequest) error {
	// Bersihkan username
...
// ValidateLoginInput memvalidasi dan membersihkan LoginRequest menggunakan validator
func ValidateLoginInput(req *domain.LoginRequest) error {
	// Trim whitespace
	...
	// Jika username terlihat seperti email, validasi format email tambahan
	if strings.Contains(req.Username, "@") {
		if !govalidator.IsEmail(req.Username) {
			return fmt.Errorf("username: invalid email format")
		}
		req.Username = strings.ToLower(req.Username)
	} else {
		// Normalisasi username ke lowercase
		req.Username = strings.ToLower(req.Username)
	}
